...
bool Graph::bfsTestConnexite(int nombreTotalSommets)
{
	...
	while (!(file.empty()))
	{
		...
		{
			//std::cout << (((s)->get_id_b()==file.front()->get_id())?(s)->get_id_a():(s)->get_id_b()) << " ";
			if (!discovered[(((s)->get_id_b() == file.front()->get_id()) ? (s)->get_id_a() : (s)->get_id_b())] && std::find(m_links.begin(), m_links.end(), s) != m_links.end())
			{
				file.push(m_points[(((s)->get_id_b() == file.front()->get_id()) ? (s)->get_id_a() : (s)->get_id_b())]);
				discovered[(((s)->get_id_b() == file.front()->get_id()) ? (s)->get_id_a() : (s)->get_id_b())] = true;
			}
		}
		//std::cout << "\n";
		file.pop();
